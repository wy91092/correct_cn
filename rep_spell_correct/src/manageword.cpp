...
ManageWord::ManageWord(const std::string &filename) : 
_words_vec(),
_words_counter() {
...
std::wstring ManageWord::findMostSimilarWord(const std::wstring &target) {
...
std::wstring ManageWord::findTopKSimilarWords(const std::wstring &target, int k) {
...
void ManageWord::readFileToVec(const std::string &filename) {
	std::ifstream is;
	is.close();
	is.clear();
	...
	while(getline(is, temp)) {
		/*将string转化成wstring*/
		std::wstring wtemp = StringToWstring(temp);
		std::wstring word;
...
int ManageWord::editDistance(const std::wstring &a, const std::wstring &b) {
	int x = a.size();
	int y = b.size();
	int memo[x + 1][y + 1];
...
std::wstring& ManageWord::cleanPunc(std::wstring &str) {
